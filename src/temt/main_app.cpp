...
MainApp::MainApp() : screen_(ScreenInteractive::Fullscreen()) {
    ...
    file_browser_->SetOnFileSelect([this](const auto& path) { file_preview_->SetFile(path); });
    ...
    // Основной layout
    auto main_container = Container::Horizontal({file_browser_, file_preview_});

    component_ = Renderer(main_container, [this] {
        return dbox({// Основной интерфейс
                     vbox({text("File Manager") | bold | center, separator(),
                           hbox({file_browser_->Render() | flex, file_preview_->Render() | flex}) | flex, separator(),
...
void MainApp::SetupCommands() {
    auto log_file = spdlog::get("file_logger");
    command_palette_->AddCommand({"open", "Open selected file",
                                  [this] {
    ...
    command_palette_->AddCommand({"navigate", "Navigate to path", [] {}, "Ctrl+P"});
